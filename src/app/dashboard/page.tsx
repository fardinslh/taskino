...
export default function DashboardPage() {
  return (
    <TaskinoApp initialView="dashboard"><DashboardPageContent /></TaskinoApp>
  );
}
...
{isManager && activeView === "dashboard" && leaveRequests.filter((lr: any) => lr.status === "pending").length > 0 && (
            <div className="overflow-hidden rounded-2xl border border-amber-200 dark:border-amber-900 bg-[--surface]">
              <div className="flex items-center justify-between border-b border-amber-100 dark:border-amber-900/50 bg-gradient-to-l from-amber-50 to-white dark:from-amber-950/30 dark:to-transparent px-5 py-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-[--text-3]">{leaveRequests.filter((lr: any) => lr.status === "pending").length} درخواست نیاز به بررسی دارد</p>
                  </div>
                </div>
              ...
              <div className="divide-y divide-[--border]">
                {leaveRequests.filter((lr: any) => lr.status === "pending").slice(0, 5).map((lr: any) => (
                ...
                <div className="grid gap-3 p-4 sm:grid-cols-2 xl:grid-cols-3">
                  {tasks.map((t: any) => (
                    <button key={getId(t)} className="rounded-xl border border-[--border] border-t-[3px] border-t-indigo-500 bg-[--surface] p-3.5 text-right shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md" onClick={() => setSelectedTask(t)} type="button">
                      <div className="flex items-center justify-between gap-2">
                        <span className={`rounded-md px-1.5 py-0.5 text-[10px] font-bold ${COLUMNS.find((c: any) => c.status === t.status)?.badge ?? "bg-slate-100 text-slate-600"}`}>{statusLabel(t.status)}</span>
